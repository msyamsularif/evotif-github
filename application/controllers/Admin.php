...
Class Admin extends CI_Controller {
	...
	public function login() {
		...
		{
			redirect('admin/loginvalidation');
		}
	...
	public function simpansekolah() {
		...
		$reg		= $this->Admin_Model->regsekolah($npsn,$nm_kampus);
		if($reg = true) {
			redirect('admin/index');
		}
		else {
	...
	public function updateidsekolah() {
		...
		$nm_rektor	= $this->input->post('nm_rektor');
		$nip			= $this->input->post('nip');
		$save			= $this->Admin_Model->updateidsekolah($npsn, $nm_kampus, $jln, $desa, $kec, $kab, $nm_rektor, $nip);
		if($save = true) {
	...
	public function hapuscalon($nim) {
		$hapus = $this->Admin_Model->hapuscalon($nim);
		if($hapus = true) {
			$this->session->set_flashdata('info', 'Berhasil Menghapus Data');
	...
	public function simpandpt() {
		...
		$save 		= $this->Admin_Model->simpandpt($username, $password, $nm_mahasiswa, $jk ,$kd_kelas);
		if($save = true) {
	...
	public function updatedpt($username){
		...
		$jk				= $this->input->post('jk');
		$kd_kelas		= $this->input->post('kd_kelas');
		$update			= $this->Admin_Model->updatedpt($username, $nm_mahasiswa, $jk, $kd_kelas);
		if($update = true) {
	...
	public function editcalon($nim) {
		$data['datacalon']	= $this->Admin_Model->datacalonspesifik($nim);
		$data['idsekolah']	= $this->Admin_Model->idsekolah();
		$this->load->view('admin/head');
		$this->load->view('admin/admin-navbar');
	...
	public function simpancalon() {
		...
		if($this->upload->do_upload('photo'))
		{
			...
			$this->Admin_Model->tambahcalon($nim, $no ,$nama, $visi, $misi, $photo);
	...
	public function updatecalon() {
		...
		$visi		= $this->input->post('visi');
		$misi		= $this->input->post('misi');
		$upade		= $this->Admin_Model->updatecalon($nim, $no ,$nama, $visi, $misi);
		if($update = true) {
			$this->session->set_flashdata('info', 'Berhasil MemperbaruiData');
			redirect('admin/editcalon/'.$nim);
		}
		else 
		{
			$this->session->set_flashdata('failed', 'Gagal Memperbarui Data');
	...
	public function cetakdaftarhadir(){
		...
		$pdf->Cell(190,7, $loaddata['nm_kampus'],0,1,'C');
		$pdf->Cell(190,7, 'Daftar Hadir Pemilihan Calon Ketua HIMATIF',0,1,'C');
		$pdf->Cell(10,7,'',0,1);
		...
		foreach($data as $load) {
			...
			$pdf->cell(70,10, $load['nm_mahasiswa'] , 1,0, 'L' );
		...
		$pdf->Cell(115,10, '',0,0, 'L');
		$pdf->Cell(70,10, 'Kepala Prodi Teknik Informatika',0,1, 'L');
		$pdf->Cell(10,20,'',0,1);
		$pdf->Cell(115,6, '',0,0, 'L');
		$pdf->SetFont('Arial','B',12);
		$pdf->Cell(70,6, $loaddata['nm_rektor'],0,1, 'L');
		$pdf->Cell(115,6, '',0,0, 'L');
		$pdf->SetFont('Arial','',12);
	...
	public function laporan(){
		...
		$pdf->SetFont('Arial','B',16);
		//HEADER LAPORAN
		$pdf->Cell(290,7, 'LAPORAN PELAKSANAAN PEMILIHAN CALON KETUA HIMATIF',0,1,'C');
		$pdf->Cell(290,7, 'TAHUN PELAJARAN '.$data['tapel'],0,1,'C');
		$pdf->Cell(10,7,'',0,1);
		$pdf->SetFont('Arial','',12);
		$pdf->Cell(60,7, 'Kota', 0,0);
		$pdf->Cell(5,7, ':', 0,0);
		...
		$pdf->Cell(90,21, 'Nama Kampus',1,0, 'C');
		...
		$pdf->Cell(90,10, $loaddata['nm_kampus'],1,0, 'C');
